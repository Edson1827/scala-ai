...
const ContractModal = ({ isOpen, onClose, selectedPlan, isAdvancedMode, setIsAdvancedMode }) => {
  const [formData, setFormData] = useState({
    ...
    // Upsells Crescimento Exponencial
    multiPlatform: false,
  ...
  // Sistema de Upsells Inteligente - 3 por Plano
  const getUpsellsByPlan = (planName) => {
    const upsells = {
      'Starter': [
        { key: 'advancedReports', name: 'Relatórios Avançados', price: 97, description: 'Dashboards personalizados e análises preditivas', icon: '📊' },
        { key: 'expressSetup', name: 'Setup Express', price: 297, description: 'Implementação em 24h com primeira campanha grátis', icon: '⚡', oneTime: true },
        { key: 'whatsappSupport', name: 'Suporte WhatsApp', price: 77, description: 'Atendimento prioritário via WhatsApp', icon: '📱' }
      ],
      'Aceleração': [
        { key: 'unlimitedCreatives', name: 'Criativos Ilimitados', price: 147, description: 'Design de anúncios profissionais ilimitados', icon: '🎨' },
        { key: 'aiOptimization', name: 'Otimização IA', price: 177, description: 'Algoritmos de machine learning para ROI +30%', icon: '🤖' },
        { key: 'dedicatedManager', name: 'Gestor Dedicado', price: 397, description: 'Gerente exclusivo para sua conta', icon: '👨‍💼' }
      ],
      'Crescimento Exponencial': [
        { key: 'multiPlatform', name: 'Multi-Plataforma', price: 197, description: 'TikTok Ads + LinkedIn Ads integrados', icon: '🌐' },
        { key: 'strategicMentoring', name: 'Mentoria Estratégica', price: 497, description: '2 calls mensais com especialista sênior', icon: '🎓' },
        { key: 'whiteLabel', name: 'White Label', price: 297, description: 'Solução com sua marca para revenda', icon: '🏷️' }
      ]
    }
    return upsells[planName] || upsells['Starter']
  }

  const planUpsells = getUpsellsByPlan(selectedPlan.name)

  // Cálculo do valor total
  const monthlyUpsells = planUpsells.filter(upsell => !upsell.oneTime && formData[upsell.key])
  const oneTimeUpsells = planUpsells.filter(upsell => upsell.oneTime && formData[upsell.key])
  
  const monthlyTotal = basePrice + monthlyUpsells.reduce((sum, upsell) => sum + upsell.price, 0)
  const oneTimeTotal = oneTimeUpsells.reduce((sum, upsell) => sum + upsell.price, 0)
      ...
      <div className="bg-white dark:bg-gray-800 rounded-lg max-w-2xl w-full max-h-[90vh] overflow-y-auto">
        <div className="p-6">
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            {/* Upsells Específicos por Plano */}
            <div className="space-y-4">
              <div className="bg-blue-50 dark:bg-blue-900/20 p-4 rounded-lg border border-blue-200 dark:border-blue-800">
                <h4 className="font-semibold text-blue-900 dark:text-blue-100 mb-3">
                  🎯 Turbine seu Plano {selectedPlan.name}
                </h4>
                
                <div className="space-y-3">
                  {planUpsells.map((upsell) => (
                    <div key={upsell.key} className="flex items-start space-x-3 p-3 bg-white dark:bg-gray-800 rounded-lg border hover:border-blue-300 transition-colors">
                      <input
                        type="checkbox"
                        id={upsell.key}
                        name={upsell.key}
                      ...
                      <div className="flex-1">
                        <div className="flex items-center justify-between">
                          <label htmlFor={upsell.key} className="font-medium text-gray-900 dark:text-white cursor-pointer flex items-center">
                            <span className="mr-2">{upsell.icon}</span>
                            {upsell.name}
                          </label>
            ...
            <div className="bg-gray-50 dark:bg-gray-700 p-4 rounded-lg">
              ...
              <div className="space-y-2 text-sm">
                ...
                <div className="flex justify-between font-bold text-lg">
                  ...
                  <span className="text-blue-600">12x de R$ {monthlyTotal}</span>
                </div>
                {oneTimeTotal > 0 && (
